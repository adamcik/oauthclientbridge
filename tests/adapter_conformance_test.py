...
async def test_adapter_preserves_safe_browser_oauth_errors(
    settings: Settings,
    adapter_client: AdapterClient,
    scenario: BrowserErrorScenario,
) -> None:
    if scenario.allowed_scopes is not None:
        settings.oauth = settings.oauth.model_copy(
            update={"allowed_scopes": scenario.allowed_scopes}
        )

    async with adapter_client(settings, None) as client:
        response = await client.get("/", params=scenario.query)

    assert response.status_code == HTTPStatus.BAD_REQUEST
    assert response.json()["error"] == scenario.error
...
@pytest.mark.parametrize("adapter_client", ["flask", "starlette"], indirect=True)
async def test_adapter_preserves_token_error_headers(
    settings: Settings, adapter_client: AdapterClient
) -> None:
    async with adapter_client(settings, None) as client:
        response = await client.post(
...
@pytest.mark.parametrize("adapter_client", ["flask", "starlette"], indirect=True)
async def test_adapter_keeps_native_routing_responses_framework_owned(
    settings: Settings, adapter_client: AdapterClient
) -> None:
    async with adapter_client(settings, None) as client:
        not_found = await client.get("/not-found")
        method_not_allowed = await client.get("/token")

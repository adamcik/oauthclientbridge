...
class Generations[T]:
    ...
    async def rotate(self, lease: "Lease[T]") -> bool:
        ...
        drain_duration: float | None = None
        rotated = False
        async with self._lock:
            ...
            if failed is self._current:
                ...
                failed.retired_at = anyio.current_time()
                rotated = True
            else:
            ...
            if failed.leases == 0 and failed is not self._current:
                finalize = failed
                drain_duration = anyio.current_time() - failed.retired_at
    ...
    async def _release_lease(self, lease: "Lease[T]") -> None:
        ...
        async with self._lock:
            ...
            if generation.leases == 0 and generation is not self._current:
                finalize = generation
                drain_duration = anyio.current_time() - generation.retired_at
